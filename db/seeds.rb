...
## TODO:
# 导入 Variety
csv = CSV.read(Rails.root.join('db', 'wine_variety.i18n.csv'))

csv.each do |item|
  puts item[1]
  Wines::Variety.create(:culture => item[1], :name => item[2], :name_en => item[3]) if Wines::Variety.find_by_name(item[2]).blank?
end
...
csv.each do |item|
  ...
  photo = Photo.create(:owner_type => 2, :business_id => @wine_detail.id, :image => File.read(file_path))
    photo = Photo.create(:owner_type => 2, :business_id => 1, :image => open('/Users/weston/Downloads/shinetech-CascadingSelectsWithBackboneJS-7bdbff3/db/images/1/2011510135148_b.jpg'))
