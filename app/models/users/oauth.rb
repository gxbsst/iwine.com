class Users::Oauth < ActiveRecord::Base
  ...
  class << self
    # Oauth Login
    def from_omniauth(auth, provider, uid)
    ...
    def build_binding_oauth(user, oauth_options = {}, qq_options = {})
    ...
    def build_oauth(user, attributes)
      #创建或者刷新绑定用户
    ...
    def has_bind_login?(sns_name, provider_user_id)
      oauth_user = oauth_login.where(:sns_name => sns_name, :provider_user_id => provider_user_id).first
      yield(oauth_user) if block_given?
      oauth_user
    end
